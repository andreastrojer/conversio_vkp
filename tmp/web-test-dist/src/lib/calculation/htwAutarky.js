...
exports.calculateHtwAutarky = calculateHtwAutarky;
const GRID = [
...
function calculateHtwAutarky({ annualDemandKwh, pvSizeKwp, usableStorageKwh, }) {
    if (!Number.isFinite(annualDemandKwh) ||
    ...
    const storageRatio = clamp(Math.max(usableStorageKwh, 0) / annualDemandMwh, 0, 2.5);
    ...
    const lowerInterpolation = q11 + (q21 - q11) * x.factor;
    const upperInterpolation = q12 + (q22 - q12) * x.factor;
    return clamp(lowerInterpolation +
        (upperInterpolation - lowerInterpolation) * y.factor, 0, 1);
}
